...
const peoplesages = people.filter(person =>{
...
function studentsScoresData(students) {
    return students.filter(studentScore => {
       const avgScore = studentScore.scores.reduce((sum, score) => sum + score, 0) / studentScore.scores.length;
       return avgScore >= 85;
    }).map(studentScore => studentScore.name);
   }
   const highAverageScore = studentsScoresData(students);
   console.log(highAverageScore);
